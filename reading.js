// draw reading chart
function drawReadingChart(data, showAnnotations = true) {
    ...
    // calculate average reading scores for each parental education level and gender
    const averages = Array.from(stackedData, ([parentEdu, gender]) => ({
        parentEdu,
        female: gender.has("female") ? d3.mean(gender.get("female"), d => d["reading score"]).toFixed(2) : 0,
        male: gender.has("male") ? d3.mean(gender.get("male"), d => d["reading score"]).toFixed(2) : 0
    }));
    ...
    // extract max avg reading scores for female and math students with their corresponding parent education levels
    const maxFemaleReading = maxFemaleData.female;
    const maxFemaleParentEdu = maxFemaleData.parentEdu;
    const maxMaleReading = maxMaleData.male;
    ...
    svg.append("text")
        ...
        .text("Students Avg Reading Scores by Gender and Parent Education");
    ...
    if(showAnnotations) {
        addAnnotations(svg, x0, x1, y, maxFemaleReading, maxFemaleParentEdu, maxMaleReading, maxMaleParentEdu);
    }
}

// add annotations
function addAnnotations(svg, x0, x1, y, maxFemaleReading, maxFemaleParentEdu, maxMaleReading, maxMaleParentEdu) {
    // define the annotations
    const annotations = [
        {
            note: {
                label: "Female Highest Avg Reading Score: " + maxFemaleReading + "\nParental Education: " + maxFemaleParentEdu,
                wrap: 100,
                wrapSplitter: /\n/, 
                align: "left"
            },
            ...
            y: y(maxFemaleReading),
            dy: 120,
            dx: 25
        },
        {
            note: {
                label: "Male Highest Avg Reading Score: " + maxMaleReading + "\nParental Education: " + maxMaleParentEdu,  
                wrap: 100,
                wrapSplitter: /\n/, 
                align: "left"
            },
